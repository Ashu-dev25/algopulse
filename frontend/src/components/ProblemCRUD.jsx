...
export default function ProblemCRUD({
  ...
  }, [statusFilter, platformFilter, difficultyFilter, search, view, onNotify]);
  ...
  const handleUpdateProblem = async (e) => {
    ...
    try {
      ...
      await problemsApi.update(editingProblem.id, {
        ...
        manual_frequency: Number(editingProblem.manual_frequency || 0),
      });
  ...
  // ============================================================
  // 8) Update tried-problem revision frequency inline
  // ============================================================
  const handleFrequencyChange = async (problem, change) => {
    if (problem.status !== "tried") return;
    const currentFrequency = Number(problem.manual_frequency || 0);
    const nextFrequency = Math.max(0, currentFrequency + change);
    if (nextFrequency === currentFrequency) return;

    try {
      await problemsApi.update(problem.id, { manual_frequency: nextFrequency });
      setProblems((currentProblems) =>
        currentProblems.map((currentProblem) =>
          currentProblem.id === problem.id
            ? { ...currentProblem, manual_frequency: nextFrequency }
            : currentProblem,
        ),
      );
    } catch (err) {
      onNotify(err.message || "Failed to update frequency.", "error");
    }
  };

  // ============================================================
  // 9) Helper: difficulty badge class
  // ============================================================
  const diffBadge = (d) =>
    d === "Easy"
      ? "badge badge-easy"
  ...
  return (
    ...
    >
      ...
      {!loading && (
        ...
        >
          {view === "today" ? "Today's" : "Tried"}: {problems.length}{" "}
      ...
      {loading ? (
        ...
        <div className="glass-panel empty-state">
          ...
          <p>
            Click "Add Problem" to log your first submission with proof URL
            validation.
          </p>
        </div>
      ) : (
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  <span
                    className={`badge ${isSolved ? "badge-solved" : "badge-tried"}`}
                  >
                    {isSolved ? "✓ Solved" : "◷ Tried"}
                  </span>
                  {!isSolved && (
                    <span className="badge badge-platform">
                      Frequency: {p.manual_frequency || 0}
                    </span>
                  )}
                </div>
                    ...
                    {!isSolved && (
                      <div className="frequency-control" aria-label="Revision frequency">
                        <button
                          ...
                          onClick={() => handleFrequencyChange(p, -1)}
                          disabled={!p.manual_frequency}
                          title="Decrease frequency"
                        >
                          <Minus size={13} />
                        </button>
                        <span>{p.manual_frequency || 0}</span>
                        <button
                          type="button"
                          className="frequency-button"
                          onClick={() => handleFrequencyChange(p, 1)}
              ...
              {status === "tried" && (
                <div>
                  ...
                  >
                    ...
                    <option value="algo_insight">Need Algorithm Insight</option>
                  </select>
                </div>
              )}
            ...
            >
              ...
              {editingProblem.status === "tried" && (
                <div>
                  <label>Blocker Reason</label>
                  <select
                    value={editingProblem.stuck_category || "wa"}
                    onChange={(e) =>
                      setEditingProblem({
                        ...editingProblem,
                        stuck_category: e.target.value,
                      })
                    }
                  >
              ...
              <div>
                <label>Manual Revision Frequency</label>
                <input
                  type="number"
                  min="0"
                  max="100000"
                  value={editingProblem.manual_frequency || 0}
                  onChange={(e) =>
                    setEditingProblem({
                      ...editingProblem,
                      manual_frequency: Number(e.target.value),
                    })
                  }
                />
              </div>
